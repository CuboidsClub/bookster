...
class TeddyController extends FlareControls {
  ...
  String submitData(data1, data2, data3) {
    print("Got details $data1 $data2 $data3");
    if (data1 != null && data2 != null && data3 != null) {
      bool emailValid =
          RegExp(r"^[a-zA-Z0-9.]+@[a-zA-Z0-9]+\.[a-zA-Z]+").hasMatch(data2);
      if (emailValid) {
        CollectionReference ref = Firestore.instance.collection('feedback');
        ref.add({"name": data1, "email": data2, "feedback": data3});
        play("success");
        print("all set");
